...
class CreateAdmitCardSettingsTable extends Migration
{
    ...
    public function up()
    {
        ...
        try {
            ...
            foreach ($leadCpmModuleIds as $key=>$data) {                   
                $check_exit=CpmModuleInfo::find($data[0]);
                if($check_exit){
                    continue;
                }
                $examPlan = new CpmModuleInfo;
                $examPlan->id = $data[0];
            ...
            foreach ($admins as $key => $value) {
                $admins_check=CpmPermissionAssign::where('module_id',$value)->where('role_id',5)->first();              
                    $permission = new CpmPermissionAssign();
                    $permission->module_id = (int)$value;
                    $permission ->module_info = CpmModuleInfo::find($value) ? CpmModuleInfo::find($value)->name : '' ;
