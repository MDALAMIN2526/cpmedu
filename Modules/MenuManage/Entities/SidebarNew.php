...
class SidebarNew extends Model
{
    ...
    {
        $check = CpmPermissionAssign::where('role_id', auth()->user()->role_id)->get('module_id');
        ...
        if (moduleStatusCheck('Saas') == True) {
            ...
            $submenu->whereNotIn('cpm_module_id', $ids);
        }
        if (auth()->user()->role_id != 1) {
            $submenu->whereIn('cpm_module_id', $check);
            $submenu->whereNotIn('cpm_module_id', [833, 834]);
        }
        if (auth()->user()->role_id == 4) {
            $submenu->whereNotIn('cpm_module_id', [833, 834]);
        }
        if (auth()->user()->role_id == 1) {
            $submenu->whereNotIn('cpm_module_id', [193]);
        }
